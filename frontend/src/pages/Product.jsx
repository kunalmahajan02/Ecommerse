...
const Product = () => {
    ...
    useEffect(() => {
        const FetchProduct = async () => {
            try {
                const res = await axios.get(`${process.env.REACT_APP_BACKENDURL1}/product/` + ProductId);
                setprod(res.data);
    ...
    useEffect(() => {
        const FetchImages = async () => {
            try {
                const res = await axios.get(`${process.env.REACT_APP_BACKENDURL1}/images/` + ProductId);
                setimage(res.data);
            } catch (err) {
                console.log("err");
            }
    ...
    useEffect(() => {
        const FetchAllProd = async () => {
            try {
                const res = await axios.get(`${process.env.REACT_APP_BACKENDURL1}/productCat/${ProductId}`)
    ...
    const handleEvent = async () => {
        ...
        const res = await axios.post(`${process.env.REACT_APP_BACKENDURL1}/AddCart`, data)
        console.log(res);
    }
    ...
    const handleSubmitfinal = async (e) => {
        ...
        fetch("http://localhost:5005/sentiment", {
            ...
            .then(data => {
                ...
                setreview("");
                console.log(data2)
                fetch(`${process.env.REACT_APP_BACKENDURL2}/addreview`, {
                    'method': 'POST',
                    headers: {
                        "Content-Type": "application/json",
                        Accept: "application/json",
            ...
            <ChakraProvider>
                <Similar style={{ padding: "100px" }}>
                    {products.map(product => (
                        <a href={`${process.env.REACT_APP_FRONTENDURL}/${custid}/product/${product.ProductId}`}>
                            <Card maxW='sm' >
                                <CardBody>
                                    <Image
                                        src={`${product.Img}`}
