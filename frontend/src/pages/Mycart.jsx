...
const Mycart = () => {
  ...
  useEffect(() => {
    const FetchAllProd = async () => {
      try {
        const res = await axios.get(`${process.env.REACT_APP_BACKENDURL1}/Cart/${custid}`)
        setproducts(res.data)

        const ress = await axios.get(`${process.env.REACT_APP_BACKENDURL1}/Cart/${custid}/1`)
    ...
    <>
      ...
      <Centre>
        <button class = " btn btn-primary" onClick={() => {
          ...
          fetch(`${process.env.REACT_APP_BACKENDURL1}/create-checkout-session`, {
            method: "POST",
            headers: {
