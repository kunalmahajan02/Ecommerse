...
app.get("/productCat/:ProductId", (req, res) => {
    const productId = req.params.ProductId;
    const q = "Select * from product where Category_Id in (select Category_Id from product p where p.ProductId = ?) and ProductId !=? order by cost DESC  limit 0, 3"
...
app.post("/AddCart", (req, res) => {
    ...
    const q = "insert into cart (CustomerId , ProductId , Quantity) values(?)"
    ...
    try {
        ...
        const Uploadstats = () => cloudinary.uploader.upload(filestr)
            .then((result) => {
                ...
                const q = `Insert into categories (CategoryName , Imgurll , catdesc) values("${name}","${result.url}","${desc}")`
                db.query(q, (err, data) => {
                    if (err) return res.json(err);
                    return res.json(data);
                })
    ...
    try {
        ...
        const Uploadstats3 =() =>  cloudinary.uploader.upload(filestr3)
            .then((result) => {
                ...
                const q4 = `Insert into images (ProductId , url) values(${productid},"${result.url}")`;
                // db.query(q2, (err, data) => {
                //     if (err) return res.json(err);
                //     // return res.json(data);
                // })
        ...
        const Uploadstats=() => cloudinary.uploader.upload(filestr1)
            .then((result) => {
                console.log("hi")
                console.log(result.url);
                const q = `Insert into product (ProductName  , Cost , Dimensions , Category_Id , Img , quantitysold , Description) values("${name}" ,${cost},"${dimensions}","${catidd}","${result.url}", "10" , "abc")`;
                db.query(q, (err, data) => {
                    if (err) return res.json(err);
                    // return res.json(data);
                })
                const q7 = `select ProductId from product order by ProductId DESC  limit 0, 1`
                db.query(q7,(err,data) =>{
                    if (err) return res.json(err);
                    console.log(data[0].ProductId);
                    productid = data[0].ProductId
                    const q2 = `Insert into images (ProductId , url) values(${productid},"${result.url}")`;
                    db.query(q2, (err, data) => {
                        if (err) return res.json(err);
                        // return res.json(data);
                        Uploadstats2()
